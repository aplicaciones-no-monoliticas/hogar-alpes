...
- El formato de `logging.basicConfig` en cada `crear_app()` y en cada `consumidor.main()` pasa de `%(levelname)s %(name)s | %(message)s` a `%(levelname)s %(name)s | cid=%(correlation_id)s | %(message)s`. Fuera de una petición o mensaje (arranque, reintentos de suscripción) se escribe `cid=-`.
...
- El BFF escribe una línea por petición con el mismo formato (método, ruta, servicio destino, código, milisegundos).
...
└── src/<paquete>/
    ...
    └── modulos/*/{aplicacion/handlers.py, infraestructura/mapeadores.py}  # los mapeadores pasan `correlacion.actual()` y se elimina el id de dominio como correlación (GT, ACR, EMP)
