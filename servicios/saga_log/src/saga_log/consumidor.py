...
def main():
    """`saga_log` observa los tres streams existentes (D6/D9 de research.md),
    cada uno en su propio hilo: `evt-trabajo-.*` (patrón, todas las regiones),
    `evt-emparejamiento`, `evt-acreditacion`. Nunca publica nada (CA-1.16)."""
    ...
    from .modulos.sagas.infraestructura.consumidores import (
        suscribirse_acreditacion,
        suscribirse_emparejamiento,
        suscribirse_trabajo,
    )

    app = crear_app()
    hilos = [
        threading.Thread(target=suscribirse_trabajo, args=(app,), daemon=True),
        threading.Thread(target=suscribirse_emparejamiento, args=(app,), daemon=True),
        threading.Thread(target=suscribirse_acreditacion, args=(app,), daemon=True),
    ]
    for hilo in hilos:
        hilo.start()
    for hilo in hilos:
