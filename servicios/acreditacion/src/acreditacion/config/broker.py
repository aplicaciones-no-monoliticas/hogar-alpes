...
_productores: dict = {}
# RLock, no Lock: `productor()` toma el candado y, sin soltarlo, llama a
# `cliente()`, que vuelve a tomarlo. Con un Lock simple eso es un
# auto-interbloqueo del mismo hilo — se manifestó como `WORKER TIMEOUT` de
# gunicorn en el primer request que publicaba un evento (el primero en crear
# el productor), nunca en el proceso consumidor porque ahí `cliente()` se
# llama antes, por separado, y el candado ya está libre.
