...
def _registrar_modelos():
    ...
    from .modulos.emparejamiento.infraestructura.dto import Base as BaseEmparejamiento
    return [BaseEmparejamiento]
...
def _registrar_rutas(app):
    ...
    from .api.candidatos import bp as bp_candidatos
...
def _crear_tablas(bases, engine):
    ...
    con `IntegrityError` sobre el catálogo (`pg_type_typname_nsp_index`), no
    con el "ya existe" que uno esperaría. Se reintenta: a la vuelta siguiente,
...
def crear_app(configuracion: dict | None = None) -> Flask:
    ...
    # `/emparejamientos/<trabajo_id>` lleva el id del trabajo; `/candidatos` no tiene id.
