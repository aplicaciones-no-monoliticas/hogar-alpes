...
def _registrar_modelos():
    ...
    from .modulos.operaciones.infraestructura.dto import Base as BaseOperaciones
...
def _registrar_handlers():
    ...
    import operaciones.modulos.operaciones.aplicacion.comandos.abrir_seguimiento  # noqa: F401
    import operaciones.modulos.operaciones.aplicacion.comandos.registrar_cambio_estado  # noqa: F401
    import operaciones.modulos.operaciones.aplicacion.queries.contar_seguimientos  # noqa: F401
    import operaciones.modulos.operaciones.aplicacion.queries.obtener_seguimiento  # noqa: F401


def _registrar_rutas(app):
    """PUNTO DE EXTENSIÓN 3 — registrar los blueprints del servicio."""
    from .api.seguimientos import bp as bp_seguimientos
    app.register_blueprint(bp_seguimientos)


def _crear_tablas(bases, engine):
    """`create_all` no es atómico frente a la carrera real de este servicio:
    `api` y `consumidor` son DOS procesos separados (§1 del plan técnico) y
    los dos llaman `crear_app()` al arrancar. Contra una base de datos recién
    creada, ambos ven "la tabla no existe" al mismo tiempo y los dos emiten
    `CREATE TABLE`; PostgreSQL deja pasar solo al primero y el segundo revienta
    con `IntegrityError` sobre el catálogo, no con el "ya existe" que uno
    esperaría. Se reintenta: a la vuelta siguiente, `create_all` ve las tablas
    del que ganó la carrera y no hace nada."""
...
def crear_app(configuracion: dict | None = None) -> Flask:
    ...
    correlacion.instalar_en_flask(app, campos_ruta={'trabajo_id': 'trabajo_id'})
