...
| **INF-4** ⚙️ ✅ | `agregar-region.sh <región>`: tópicos y suscripciones de una región nueva, con la **misma definición** que usa la inicialización (`crear_region` en `comun.sh`) | CA-8.4 *(30 pt)* | INF-3 | ✅ `conosur` creada con 4 particiones y sus 3 suscripciones, verificadas contra el broker · segunda corrida idempotente · `andina` y `norteamerica` intactas | 30 min |
| **INF-5** ⚙️ ✅ | 4 PostgreSQL, **redes Docker separadas**, `.env.example` | CA-T1 · RNF-1 *(topología 5 pt + comunicación 20 pt)* | INF-1 | ✅ **9 de 9** comprobaciones: cada servicio resuelve solo su propia base; `postgres-acreditacion` no existe para `gestion-trabajos` | 1 h |
...
| ID | Tarea | Criterio | Depende de | Verificación | Est. |
|---|---|---|---|---|---|
| **GT-1** ✅ | Separar API y consumidor: gunicorn para la API, proceso aparte para el consumidor, quitar el hilo de `create_app` | RT-5 · habilita CA-8.3 *(30 pt)* | INF-1, INF-6 | ✅ Dos contenedores arriba · `/health` responde con su modo · el consumidor **reintentó 18 veces** contra un broker inexistente sin morir | 1 h |
| **GT-2** ✅ | Despachador nuevo: cliente y productor reutilizados por proceso, `partition_key`, propiedades `partner_id`, `region`, `correlation_id` | G-3a · RNF-4 · habilita CA-8.3 *(30 pt)* | CON-1, INF-6 | ✅ 10 `POST` → 10 eventos publicados · **un productor por proceso** (`WORKERS=1` → 1, 2 workers → 2, no 10) · colección 36/36 | 1,5 h |
...
| **GT-4** ✅ | `CrearTrabajo` con `trabajo_id` opcional + creación **idempotente** + consumidor por patrón `cmd-trabajo-.*` (Shared) | CA-E1 · RNF-4 *(esquemas 5 pt + 30 pt)* | GT-3 | ✅ `test_aplicacion_trabajos.py`: el mismo comando dos veces crea **un** trabajo · el contrato se migró al plano de CON-1 (`trabajo_id` incluido) | 1,5 h |
| **GT-5** ✅ | **Retirar el módulo `operaciones`**: código, modelos, `create_all` y el endpoint `/seguimiento` | **G-6** · RNF-1 *(comunicación, 20 pt)* | OPS-3 | ✅ `pytest` verde · 0 referencias a `modulos.operaciones` en el servicio | 45 min |
| **GT-6** ✅ | **MOD-1**: `RepositorioTrabajosMemoria` + selección por `ADAPTADOR_TRABAJOS`, usado por las pruebas de aplicación | **CA-M1** · **G-5** *(30 pt)* | GT-1 | ✅ Corre con `memoria` (`pytest`) · `escenarios/mod-1.sh` verifica `git diff --stat` sobre `dominio/`/`aplicacion/` y corre Postman — pendiente de ejecutarse contra un clúster real | 1,5 h |
| **GT-7** ✅ | Regresión: pruebas y colección de GT en verde tras todos los cambios | **CA-M4** *(30 pt)* | GT-3…GT-6 | ✅ `pytest` 12/12 (se agregó `tests/conftest.py`, faltaba) · **`newman` contra el sistema real pendiente** — no había Docker en este entorno | 30 min |
...
| **OPS-2** ✅ | **Consumidor Pulsar**: patrón `evt-trabajo-.*`, Failover, **capa anticorrupción** evento → comando, Unidad de Trabajo propia, `ack` después del commit, `nack` ante excepción | **G-1** · CA-6.4 · RNF-1 · RNF-4 *(30 pt + 20 pt)* | OPS-1, CON-1, INF-3 | ✅ `pytest` con eventos sintéticos (`test_consumidor.py`): `TrabajoCreado` abre el seguimiento, `EstadoTrabajoCambiado` lo actualiza · GT-3 ya publica el contrato real en `evt-trabajo-{región}` (`docs/decisiones.md`, sección GT-3): falta correr `escenario-6.sh` de punta a punta contra un clúster real para la verificación end-to-end | 3 h |
...
| **ESC-6** ⚙️ ✅ | `escenario-6.sh`: línea base → detener el consumidor → carga → backlog → reanudar → conteos | S | **CA-6.1 … CA-6.6** *(30 pt)* | OPS-3, HER-1, INF-3 | Simplificado tras GT-3: un solo generador (`--via-http --con-cambios-estado`), sin el atajo sintético que usaba antes · sintaxis verificada (`bash -n`) · **corrida con *T* = 3 min contra un clúster real pendiente** — no había Docker disponible en este entorno (ver `docs/decisiones.md`) | 2,5 h |
...
| **ESC-M** ⚙️ ✅ | `mod-1.sh`, `mod-2.sh`, `mod-3.sh` | A | **CA-M1 · CA-M2 · CA-M3** *(30 pt)* | GT-6, GT-3, OPS-2 | Escritos, con sintaxis verificada · **corrida contra un clúster real pendiente** — no había Docker en este entorno | 2 h |
| **ESC-E** ⚙️ ✅ | `esquemas.py`: campo nuevo aceptado y esquema incompatible rechazado | A | **CA-E1 · CA-E2** *(esquemas, 5 pt)* | INF-0, GT-4 | Escrito contra el contrato real (`contratos/v1/cmd_trabajo.py`) · las clases de esquema se verificaron sin broker · **corrida contra el registro real pendiente** | 1 h |
...
| ID | Tarea | Dueño | Criterio | Depende de | Verificación | Est. |
|---|---|---|---|---|---|---|
| **INT-1** ⚙️ ✅ | `docker-compose.yml` completo: Pulsar, 4 PostgreSQL, 4 API y los consumidores | A | **RNF-3** *(código que corre — 30 pt)* | GT-7, OPS-4, ACR-5, EMP-5 | ✅ Arranque **desde volúmenes vacíos**: las 4 API responden, solo salen `pulsar-init` y `pulsar-config` (código 0, por diseño) y la topología se recrea sola: `PASA`, 7 tópicos y 9 suscripciones | 2 h |
...
| **DEP-1** ⚙️ ✅ | AWS: EC2 t3.xlarge, security group, `user-data.sh`, despliegue, `infra/aws/README.md` | A | **RNF-6** *(despliegue, 5 pt)* | INT-1 | `infra/aws/user-data.sh` + `infra/aws/README.md` escritos · **despliegue real en una EC2 pendiente** — no hay credenciales de AWS en este entorno | 2 h |
...
| **DOC-1** ⚙️ ✅ | `README.md`: escenarios, estructura, cómo desplegar, actividades | A | RNF-6 *(README — requisito de la guía)* | INT-1 | ✅ Reescrito para los cuatro servicios: arquitectura, cómo levantar el sistema, tabla de escenarios con su script, estructura del repositorio | 1,5 h |
| **DOC-2** ⚙️ | `docs/decisiones.md`: tipos de evento, esquemas, topología de datos, CRUD vs ES, clúster, DDD | A, S, J | §6 y §7 de la especificación *(tipos de evento 5 pt + topología 5 pt)* | INT-1 | Los seis ítems ya están cubiertos, repartidos entre `docs/decisiones.md` (implementación) y `docs/01-especificacion.md` §4/§6/§7 (arquitectura) — **falta la pasada final de consolidación entre los tres**, que por diseño es de cierre de entrega, no de una tarea aislada | 2 h |
| **DOC-3** ⚙️ ✅ | `docs/actividades.md` con enlaces a commits y PR | A, S, J | *(actividades por miembro, 5 pt)* | DEP-2 | ✅ Borrador con los tres integrantes y sus PR/commits reales · **la fila de Stiven (`stiven/cierre-escenario-6`) queda como rama hasta que se abra su PR** — la de Andrés ya se fusionó en el PR #12 | 45 min |
| **DOC-4** ⚙️ | Ensayo: cada uno defiende **una decisión que no implementó** | A, S, J | Sustentación *(70% de la nota)* | DOC-2 | Es un ensayo en vivo entre los tres integrantes — no es una tarea de código, queda para antes de la sustentación | 1 h |
